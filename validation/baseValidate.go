...
var zeroOrOne validation.CustomFunc = func(v *validation.Validation, obj interface{}, key string) {
	objType := reflect.TypeOf(obj).String()
	if objType != "uint8" {
		v.AddError(key, "传递参数错误")
		return
	}
	data := obj.(uint8)
	if data != 0 && data != 1 {
...
func ValidateInit(){
	validation.AddCustomFunc("ZeroOrOne",zeroOrOne)
	if len(MessageTmpls) == 0 {
		return
	}
	for k := range MessageTmpls {
